...
export default function SavagesGear() {
  return <main className="savages-gear-page">
    ...
    <section className="gear-feature" aria-labelledby="last-cast-title"><div className="gear-feature-copy"><p>FEATURED ARTWORK / 01</p><h2 id="last-cast-title">LAST<br />CAST.</h2><span>Sticker</span><a href="#collection">View Sticker <b>→</b></a></div><figure><Image src="/assets/savages-sticker-last-cast.png" alt="Last Cast Savages Fly Gear sticker artwork" fill sizes="(max-width: 800px) 100vw, 52vw" priority /></figure></section>

    <section id="collection" className="gear-collection" aria-labelledby="collection-title"><div className="gear-collection-heading"><p>THE STICKER BIN / 07</p><h2 id="collection-title">Built for boxes,<br />boats, and bad ideas.</h2></div><div className="gear-art-grid">{stickers.map((sticker) => <article className={sticker.className} key={sticker.name}><div><Image src={sticker.image} alt={sticker.alt} fill sizes="(max-width: 600px) 100vw, (max-width: 1000px) 50vw, 33vw" /></div><p>{sticker.name} <span>Sticker</span></p></article>)}</div></section>
